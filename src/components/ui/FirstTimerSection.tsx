...
import { Send, User, Phone, Mail, MapPin, Briefcase, Calendar } from 'lucide-react'
...
const formSchema = z.object({
  ...
  isFirstTime: z.string().min(1, 'Please indicate if this is your first time'),
  prayerRequest: z.string().optional(),
  preferredFollowUp: z.string().min(1, 'Please select a follow-up method'),
})

type FormValues = z.infer<typeof formSchema>

export function FirstTimerSection() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      /*
        ...
        1. Replace the URL `/api/first-timers` with your actual backend endpoint 
           or use an env variable.
        2. Ensure your backend accepts a POST request with a JSON body matching 
           the `data` payload structure.
      */

      const apiUrl = process.env.NEXT_PUBLIC_API_URL 
        ? `${process.env.NEXT_PUBLIC_API_URL}/first-timers` 
        : '/api/first-timers' // Fallback for testing
  ...
  return (
    <section className="relative w-full py-24 bg-white overflow-hidden" id="first-timer">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-6 sm:px-10 lg:px-20">
        ...
        <div className="text-center mb-16 max-w-2xl mx-auto">
          ...
          >
            Welcome <span className="text-[#DDB771]">Home</span>
          </motion.h2>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            We are so glad you are here! Please take a moment to fill out this form so we can get to know you better and stay connected.
          </motion.p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-6 border-t border-gray-100">
              ...
              <div>
                ...
                {errors.maritalStatus && <p className="text-red-500 text-xs mt-2">{errors.maritalStatus.message}</p>}
              </div>
            ...
            <div className="space-y-8 pt-6 border-t border-gray-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <label className={labelClassName}>Name of Inviter (If any)</label>
                  <input {...register('inviterName')} className={inputClassName} placeholder="Who invited you?" />
                </div>
                
                <div>
                  <label className={labelClassName}>Is this your first time at RCCG SC? *</label>
                  <div className="flex gap-6 mt-4">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <input type="radio" value="Yes" {...register('isFirstTime')} className="w-5 h-5 text-[#DDB771] focus:ring-[#DDB771] border-gray-300" />
                      <span className="text-gray-700 group-hover:text-[#DDB771] transition-colors">Yes</span>
                    </label>
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <input type="radio" value="No" {...register('isFirstTime')} className="w-5 h-5 text-[#DDB771] focus:ring-[#DDB771] border-gray-300" />
                      <span className="text-gray-700 group-hover:text-[#DDB771] transition-colors">No</span>
                    </label>
                  </div>
                  {errors.isFirstTime && <p className="text-red-500 text-xs mt-2">{errors.isFirstTime.message}</p>}
                </div>
              </div>

              <div>
                <label className={labelClassName}>Prayer Request</label>
                <textarea 
                  {...register('prayerRequest')} 
                  className={`${inputClassName} min-h-[120px] resize-y`} 
                  placeholder="How can we pray with you? (Optional)" 
                />
              </div>

              <div>
                <label className={labelClassName}>Preferred Follow-up Method *</label>
                <select {...register('preferredFollowUp')} className={inputClassName}>
                ...
                {errors.preferredFollowUp && <p className="text-red-500 text-xs mt-2">{errors.preferredFollowUp.message}</p>}
              </div>
            </div>
