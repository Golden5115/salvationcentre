...
import { adminGetSpecialEvents, deleteSpecialEvent } from "@/lib/api/special-events"
...
export default function AdminEventsPage() {
  ...
  const fetchEvents = async () => {
    const t = toast.loading("Loading events...")
    try {
      setLoading(true)
      const response = await adminGetSpecialEvents()
      if (response.success && Array.isArray(response.data)) {
        setEvents(response.data)
        toast.dismiss(t)
        toast.success(`Loaded ${response.data.length} event(s)`)
      } else {
        setEvents([])
        toast.dismiss(t)
  ...
  const handleDelete = async (id: number, title: string) => {
    ...
    try {
      const response = await deleteSpecialEvent(id)
      if (response.success) {
      ...
      <div className="min-h-screen bg-white relative overflow-hidden">
        ...
        <div className="relative z-20 flex items-center justify-center min-h-screen">
          <div className="text-white flex flex-col items-center gap-6">
            ...
            <div>
              <p className="text-2xl font-black mb-2 text-center">Loading Events</p>
            ...
            <div className="p-6 lg:p-8">
              ...
              <div className="mb-10">
                ...
                <h1 className="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight">
                  Special<br />
                  <span className="italic font-light">Events</span>
                </h1>
                <div className="w-24 h-1 bg-white mb-6"></div>
                <p className="text-lg text-white/80 max-w-3xl font-light">
                  Manage church special events, services, and programs.
                </p>
              </div>
              ...
              {!loading && filteredEvents.length === 0 && (
                <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-12 border-4 border-black text-center">
                  ...
                  <h3 className="text-3xl font-black mb-4 text-black">
                    {events.length === 0 ? "No special events yet" : "No events found"}
                  </h3>
                  <p className="text-gray-700 text-lg max-w-md mx-auto mb-6">
                    {events.length === 0
                      ? "Start by creating your first special event for the church"
